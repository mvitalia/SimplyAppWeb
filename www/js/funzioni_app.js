...
$(document).ready(function () {
    ...
    $('#pubblicaMarchio').validate({
        ...
        submitHandler: function (form) {
            if ($('#sito').is(":checked")) {
                ...
                var posizione = $("#posizione_marchio").val();
                var nome = $("#nome_marchio").val();
                var nome_en = $("#nome_en_marchio").val();
                var nome_fr = $("#nome_fr_marchio").val();
                var link = $("#link_marchio").val();
                tinyMCE.triggerSave();
                var descrizione_breve = $("#descrizione_breve_marchio").val();
                var descrizione_breve_en = $("#descrizione_breve_en_marchio").val();
                var descrizione_breve_fr = $("#descrizione_breve_fr_marchio").val();
             
                caricaSulSitoMarchio(posizione, nome, nome_en, nome_fr, descrizione_breve, descrizione_breve_en, descrizione_breve_fr, link);
            } else {
                var pubblica_sito_check = "<div class='modal-content'><h4>Condivisione Fallita </h4><p>Clicca su checkbox sito per pubblicare!! </p></div>";
                pubblica_sito_check += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#scegli_pubblica_sito").html("");
    ...
    $('#pubblicaMarchioModifica').validate({
        rules: {

            posizione_marchio_modifica: {
                required: true,

            },
            nome_marchio_modifica: {
                required: true,
            }
        },
        messages: {
        ...
        submitHandler: function (form) {
            var posizione = $("#posizione_marchio_modifica").val();
            var nome = $("#nome_marchio_modifica").val();
            var nome_en = $("#nome_en_marchio_modifica").val();
            var nome_fr = $("#nome_fr_marchio_modifica").val();
            var link = $("#link_marchio_modifica").val();
            tinyMCE.triggerSave();
            var descrizione_breve = $("#descrizione_breve_marchio_modifica").val();
            var descrizione_breve_en = $("#descrizione_breve_en_marchio_modifica").val();
            var descrizione_breve_fr = $("#descrizione_breve_fr_marchio_modifica").val();
          
            aggiornaMarchio(posizione, nome, nome_en, nome_fr, descrizione_breve, descrizione_breve_en, descrizione_breve_fr, link);
            // Quando torno vai alla funzione aggiorna argomento e modifica con descrizione breve e poi webservice collegato
            return false;
        }
    });
        ...
        $.each(dati, function (i, utente) {
            ...
            db.transaction(
                ...
                function (tx) {
                    ...
                    tx.executeSql("INSERT INTO utenti (identificativo,social,sito,news,promo,eventi,gallery,prodotti,servizi,realizzazioni,serviziCat,certificazioni,appartamenti,prenotazioni,approfondimenti,marchi) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [utente.ID, utente.checkSocial, utente.checkSito, utente.checkNews, utente.checkPromo, utente.checkEventi, utente.checkGallery, utente.checkProdotti, utente.checkServizi, utente.checkRealizzazioni, utente.checkServiziCat, utente.checkCertificazioni, utente.checkAppartamenti, utente.checkPrenotazioni, utente.checkApprofondimenti, utente.checkMarchi]);
                },
                function (e) {
        ...
        tx.executeSql("SELECT u.identificativo, u.social, u.sito, u.news, u.eventi, u.promo, u.gallery, u.prodotti, u.servizi, u.realizzazioni, u.serviziCat, u.certificazioni, u.appartamenti, u.prenotazioni, cu.piattaforma, cu.ID_utente FROM utenti as u, configurazione_utente as cu WHERE u.identificativo = cu.ID_utente AND u.Identificativo=" + localStorage.getItem('ID_utente') + "", [],
         function (tx, dati) {
             ...
             if (len != 0) {
                 ...
                 if (dati.rows.item(0).marchi == 1) {
                     tab_argomenti += " <button onclick='clickMarchi()' class='filter' id='realizzazioni' >Marchi</button>"
                     count++;
        ...
        tx.executeSql("SELECT * FROM utenti WHERE identificativo = " + localStorage.getItem('ID_utente') + "", [],
         function (tx, dati) {
             ...
             if (len != 0) {
                 ...
                 if (dati.rows.item(0).marchi == 1) {
                     tab_argomenti += " <button onclick='clickMarchi()' class='filter' id='marchi' >Marchi</button>"
                     count++;
...
function clickMarchi() {
    localStorage.setItem('argomento', 'Marchi');
  
    $.mobile.pageContainer.pagecontainer("change", "#marchi", {
        transition: 'flip',
    });
    ...
    var count = 0;
     ...
     function (tx, dati) {
         ...
         if (len != 0) {
             ...
             if (dati.rows.item(0).marchi == 1) {
                 tab_argomenti += " <button onclick='clickMarchiStorico()' class='filter' id='marchi' >Marchi</button>"
                 count++;
...
function clickMarchiStorico() {

    $.mobile.pageContainer.pagecontainer("change", "#storicoMarchi", {
        transition: 'flip',
    });
}
...
$(document).on("pagebeforeshow", "#marchi", function () {
   
    $(".appendi_lingue").html("");
    $("#appendi_img_marchio").html("");
    $("#nome_marchio").val("");
    $("#nome_en_marchio").val("");
    $("#nome_fr_marchio").val("");
    $("#posizione_marchio").val("");
    $("#link_marchio").val("");
    tinyMCE.get('descrizione_breve_marchio').setContent("");
    tinyMCE.get('descrizione_breve_en_marchio').setContent("");
    tinyMCE.get('descrizione_breve_fr_marchio').setContent("");
   
    selezionoLingue();
});

$(document).on("pageshow", "#marchi", function () {
    // Seleziono i dati delle categorie della gallery
    var foto = "<img class='circle avatar' src='http://simplyappweb.mvclienti.com/Account/fotoOperatori/" + localStorage.getItem("foto_utente") + "' rel='external' alt=''>";
    $(".appendi_foto_utente_nome").html("");
    $(".appendi_foto_utente_nome").append(foto);
...
$(document).on("pagebeforeshow", "#storicoMarchi", function () {
    // Ripulisco la pagina dalle parti dinamiche
    $("#si_fixed_footer_marchi").show();
    $("#no_fixed_footer_marchi").hide();
    $("#stato_attivo_filtro").removeClass("active");
    $("#stato_non_attivo_filtro").removeClass("active");
    $("#ricerca").val("");
    $(".appendi_marchi").html("");
    localStorage.setItem("argomento_filtri", "marchi");



});

$(document).on("pageshow", "#storicoMarchi", function () {
    // Seleziono i dati delle categorie della gallery
    var foto = "<img class='circle avatar' src='http://simplyappweb.mvclienti.com/Account/fotoOperatori/" + localStorage.getItem("foto_utente") + "' rel='external' alt=''>";
    $(".appendi_foto_utente_nome").html("");
    $(".appendi_foto_utente_nome").append(foto);
...
$(document).on("pagebeforeshow", "#modifica_marchio", function () {
    var foto = "<img class='circle avatar' src='http://simplyappweb.mvclienti.com/Account/fotoOperatori/" + localStorage.getItem("foto_utente") + "' rel='external' alt=''>";
    $(".appendi_foto_utente_nome").html("");
    $(".appendi_lingue").html("");
    $(".appendi_foto_utente_nome").append(foto);
    ...
    $(".appendi_img_modifica_marchio").html("");
    selezionoMarchioModifica();
    selezionoLingue();

});

function caricaSulSitoMarchio(posizione, nome, nome_en, nome_fr, descrizione_breve, descrizione_breve_en, descrizione_breve_fr, link) {

 

    $.ajax({
        type: "POST",
        data: '{posizione:"' + posizione + '", nome:"' + nome + '", nome_en:"' + nome_en + '", nome_fr:"' + nome_fr + '", descrizione_breve:"' + descrizione_breve + '", descrizione_breve_en:"' + descrizione_breve_en + '", descrizione_breve_fr:"' + descrizione_breve_fr + '",link:"' + link + '", ID_utente:"' + localStorage.getItem("ID_utente") + '", user:"' + localStorage.getItem("username") + '"}',
        url: 'http://simplyappweb.mvclienti.com/webservices/carica_argomento_app.aspx/caricaMarchio',
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (data) {
            var ritorno = data.d;
            ...
            if (ritorno == "Si") {
                var siCaricamento = "<div class='modal-content'><h4>Marchio Caricato </h4><p>Marchio caricato con successo</p></div>";
                siCaricamento += "<div class='modal-footer'> <a href='#storicoMarchi'  class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_caricamento_marchio").html("");
                $("#box_caricamento_marchio").append(siCaricamento);
                $("#apri_box_caricamento_marchio").click();
            } else {
                var noCaricamento = "<div class='modal-content'><h4>Marchio non caricato </h4><p>Marchio non caricato.<br>Si prega di riprovare </p></div>";
                noCaricamento += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_caricamento_marchio").html("");
                $("#box_caricamento_marchio").append(noCaricamento);
                $("#apri_box_caricamento_marchio").click();
            }
...
function recuperaMarchi() {
    $(".appendi_marchi").html("");
    // Faccio la selezione dal server delle new di questo utente
    $.getJSON("http://simplyappweb.mvclienti.com/webservices/storico_marchi.aspx?id_utente=" + localStorage.getItem("ID_utente") + "", function (dati) {
        var blocco_marchi = "";
        var lunghezza = dati.length;
        var Realizzazioni = "Marchi";

        $.each(dati, function (i, marchi) {


            blocco_marchi += "<div class='blog-card animated fadein delay-1'><div class='blog-header'><div class='blog-author'><span class='titolo_storico'>" + marchi.nome + "</span></div></div>";

            blocco_marchi += "<div class='blog-image'>  <img src='http://simplyappweb.mvclienti.com/Account/Uploads/Marchi/galleria_img/immagini/" + marchi.immagine + "' rel='external' alt=''/></div>";

            blocco_marchi += "<div class='blog-preview p-20'><p>" + marchi.descrizione_breve + "</p><div class='btn_storico'><a href='#' onclick='modificaMarchio(\"" + marchi.ID + "\");' class='waves-effect waves-light btn accent-color'><span class='ion-edit'></span></a><a href='#' onclick='cancellaMarchio(\"" + marchi.ID + "\");' class='waves-effect waves-light btn accent-color'><span class='ion-close-circled'></span></a></div></div></div>";
        });
        if (lunghezza == 0) {
            blocco_marchi += "<div class='modal-content'><h4>No Marchi </h4><p>Non vi sono marchi salvati!!</p></div>";
            blocco_marchi += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
            $("#no_marchi").html("");
            $("#no_marchi").append(blocco_marchi);
            $("#apri_no_marchi").click();
        }

        if (lunghezza != 0) {

            $(".appendi_marchi").append(blocco_marchi);
            $("#si_fixed_footer_marchi").hide();
            $("#no_fixed_footer_marchi").show();
        }

    });
}

function cancellaMarchio(ID_servizio) {
    $.ajax({
        type: "POST",
        data: '{ID:"' + ID_servizio + '", ID_utente:"' + localStorage.getItem("ID_utente") + '"}',
        url: 'http://simplyappweb.mvclienti.com/webservices/servizi_app.aspx/cancellaMarchio',
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (data) {
            var ritorno = data.d;
            if (ritorno == "si") {
                var siCancellaNews = "<div class='modal-content'><h4>Marchio eliminato </h4><p>Marchio eliminato con successo!!</p></div>";
                siCancellaNews += "<div class='modal-footer'> <a href='#' onclick='recuperaMarchi()' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_cancellazione_marchio").html("");
                $("#box_cancellazione_marchio").append(siCancellaNews);
                $("#apri_box_cancellazione_marchio").click();
            } else {
                var noCancellaNews = "<div class='modal-content'><h4>Marchio non eliminato </h4><p>Marchio non eliminato!!<br>Si prega di riprovare. </p></div>";
                noCancellaNews += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_cancellazione_marchio").html("");
                $("#box_cancellazione_marchio").append(noCancellaNews);
                $("#apri_box_cancellazione_marchio").click();
            }
...
function modificaMarchio(ID) {

    localStorage.setItem('id_modifica_argomento', ID);
    $.mobile.pageContainer.pagecontainer("change", "#modifica_marchio", {
        transition: 'flip',
...
function selezionoMarchioModifica() {
  
    var ID = localStorage.getItem("ID_utente");
    var id_prodotto = localStorage.getItem('id_modifica_argomento');
    $.getJSON("http://simplyappweb.mvclienti.com/webservices/seleziono_marchio.aspx?id_servizio=" + id_prodotto + "&ID_utente=" + ID + "", function (dati) {
        // var blocco_news = "";
        // Continuo da qui
        var lunghezza = dati.length;
        ...
        $.each(dati, function (i, argomento) {
            tinyMCE.triggerSave();
            $("#lbl_marchio_posizione").addClass("active");
            $("#posizione_marchio_modifica").val(argomento.posizione);
            $("#lbl_marchio_nome").addClass("active");
            $("#nome_marchio_modifica").val(argomento.nome);
            $("#lbl_marchio_nome_en").addClass("active");
            $("#nome_en_marchio_modifica").val(argomento.nome_en);
            $("#lbl_marchio_nome_fr").addClass("active");
            $("#nome_fr_marchio_modifica").val(argomento.nome_fr);
            $("#lbl_marchio_link").addClass("active");
            $("#link_marchio_modifica").val(argomento.link);
            tinyMCE.get('descrizione_breve_marchio_modifica').setContent(argomento.descrizione_breve);
            tinyMCE.get('descrizione_breve_en_marchio_modifica').setContent(argomento.descrizione_breve_en);
            tinyMCE.get('descrizione_breve_fr_marchio_modifica').setContent(argomento.descrizione_breve_fr);
            if (argomento.immagine != "") {
                immagine = "<div class='grid-item gallery-item-card ' id='" + argomento.ID + "'> <img src='http://simplyappweb.mvclienti.com/Account/Uploads/Marchi/galleria_img/immagini/" + argomento.immagine + "' rel='external' alt='image'>";
                immagine += "<div onclick='cancellaFotoMarchio(\"" + argomento.ID + "\");'  class='gallery-item-header'><div class='gallery-item-author'><span><i class='ion-close-circled'></span></div></div></div>";
...
function cancellaFotoMarchio(ID) {
    ...
    $.ajax({
        type: "POST",
        data: '{ID:"' + ID + '",ID_utente: "' + localStorage.getItem('ID_utente') + '"}',
        url: 'http://simplyappweb.mvclienti.com/webservices/servizi_app.aspx/cancelloMediaMarchio',
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (data) {
            var ritorno = data.d;
            ...
            if (ritorno == "si") {
                var siCancellaMediaArgomento = "<div class='modal-content'><h4>Foto eliminata </h4><p>Foto eliminta con successo!!</p></div>";
                siCancellaMediaArgomento += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_cancellazione_media_marchio").html("");
                $("#box_cancellazione_media_marchio").append(siCancellaMediaArgomento);
                $("#apri_box_cancellazione_media_marchio").click();
            } else {
                var noCancellaMediaArgomento = "<div class='modal-content'><h4>Foto non eliminata </h4><p>Foto non eliminata!!<br>Si prega di riprovare. </p></div>";
                noCancellaMediaArgomento += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_cancellazione_media_marchio").html("");
                $("#box_cancellazione_media_marchio").append(noCancellaMediaArgomento);
                $("#apri_box_cancellazione_media_marchio").click();
            }
...
function aggiornaMarchio(posizione, nome, nome_en, nome_fr, descrizione_breve, descrizione_breve_en, descrizione_breve_fr, link) {



    $.ajax({
        type: "POST",
        data: '{posizione:"' + posizione + '",nome:"' + nome + '",nome_en:"' + nome_en + '",nome_fr:"' + nome_fr + '", descrizione_breve:"' + descrizione_breve + '",descrizione_breve_en:"' + descrizione_breve_en + '",descrizione_breve_fr:"' + descrizione_breve_fr + '",link:"' + link + '", ID_utente:"' + localStorage.getItem("ID_utente") + '", ID_servizio: "' + localStorage.getItem('id_modifica_argomento') + '"}',
        url: 'http://simplyappweb.mvclienti.com/webservices/aggiorno_prodotto.aspx/updateMarchio',
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        success: function (data) {
            var ritorno = data.d;
            ...
            if (ritorno == "Si") {
                var siCaricamento = "<div class='modal-content'><h4>Marchio Aggiornato </h4><p>Marchio aggiornato correttamente</p></div>";
                siCaricamento += "<div class='modal-footer'> <a href='#dashboard' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_aggiornamento_marchio").html("");
                $("#box_aggiornamento_marchio").append(siCaricamento);
                $("#apri_box_aggiornamento_marchio").click();
            } else {
                var noCaricamento = "<div class='modal-content'><h4>Marchio non aggiornato </h4><p>Marchio non aggiornato.<br>Si prega di riprovare </p></div>";
                noCaricamento += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
                $("#box_aggiornamento_marchio").html("");
                $("#box_aggiornamento_marchio").append(noCaricamento);
                $("#apri_box_aggiornamento_marchio").click();
            }
...
$("#flip-storico-marchi").bind("change", function (event, ui) {
    $("#filtri_marchi").toggle();
    var stato_flip = $("#flip-storico-marchi").prop("checked");
    if (stato_flip == true) {
...
function filtraArgomentoMarchio(stato, tipo) {


    if (stato == "si" && tipo == "Attivo") {
        $("#stato_attivo_filtro_marchi").addClass("active");
        $("#stato_non_attivo_filtro_marchi").removeClass("active");
    }
    if (stato == "si" && tipo == "Non Attivo") {
        $("#stato_attivo_filtro_marchi").removeClass("active");
        $("#stato_non_attivo_filtro_marchi").addClass("active");
    }

    var argomento_giusto = "marchi"
    var ricerca_valore = $("#ricerca_marchio").val();
    if (ricerca_valore != "") {
        ricerca = ricerca_valore;
    } else {
        ricerca = "";
    ...
    var argomento = "marchi";
   
    $(".appendi_marchi").html("");

    $.getJSON("http://simplyappweb.mvclienti.com/webservices/storico_marchi_filtri.aspx?id_utente=" + localStorage.getItem("ID_utente") + "&stato=" + stato + "&ricerca=" + ricerca + "&tipo_argomento=" + argomento + "&tipo_stato=" + tipo + "", function (dati) {
        var blocco_marchi = "";
        var lunghezza = dati.length;
        ...
        $.each(dati, function (i, argomenti_filtrati) {
            ...
            blocco_marchi += "<div class='blog-card animated fadein delay-1'><div class='blog-header'><div class='blog-author'><span class='titolo_storico'>" + argomenti_filtrati.nome + "</span> </div></div>";

            blocco_marchi += "<div class='blog-image'>  <img src='http://simplyappweb.mvclienti.com/Account/Uploads/Marchi/galleria_img/immagini/" + argomenti_filtrati.immagine + "' rel='external' alt=''/></div>";

            blocco_marchi += "<div class='blog-preview p-20'><p>" + argomenti_filtrati.descrizione_breve + "</p><div class='btn_storico'><a href='#' onclick='modificaMarchio(\"" + argomenti_filtrati.ID + "\");' class='waves-effect waves-light btn primary-color'><span class='ion-edit'></span></a><a href='#' onclick='cancellaMarchio(\"" + argomenti_filtrati.ID + "\");' class='waves-effect waves-light btn primary-color'><span class='ion-close-circled'></span></a></div></div></div>";
        });
        if (lunghezza == 0) {
            blocco_marchi += "<div class='modal-content'><h4>No Marchi </h4><p>Nessun marchio  trovata dopo la ricerca!!</p></div>";
            blocco_marchi += "<div class='modal-footer'> <a href='#' class='modal-action modal-close waves-effect waves-green btn-flat'>Chiudi</a></div>";
            $("#no_marchi").html("");
            $("#no_marchi").append(blocco_marchi);
            $("#apri_no_marchi").click();
